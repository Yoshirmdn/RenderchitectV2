  ...
  if (wishlist.length === 0) {
    return (
      <div className="min-h-screen pt-32 flex flex-col items-center justify-center px-6" style={{ background: "var(--bg-primary)" }}>
        ...
        <h2 className="font-heading text-2xl font-bold mb-3" style={{ color: "var(--text-primary)" }}>Your wishlist is empty</h2>
        <p className="mb-8" style={{ color: "var(--text-muted)" }}>Save designs you love to revisit later</p>
    ...
    <div className="min-h-screen pt-24 pb-16" style={{ background: "var(--bg-primary)" }}>
      <div className="max-w-6xl mx-auto px-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-10">
          <p className="font-mono text-sm tracking-widest uppercase mb-2" style={{ color: "var(--accent)" }}>Saved Items</p>
          <h1 className="font-display text-5xl" style={{ color: "var(--text-primary)" }}>YOUR WISHLIST</h1>
          <p className="mt-1" style={{ color: "var(--text-muted)" }}>{wishlist.length} saved design{wishlist.length !== 1 ? "s" : ""}</p>
        </motion.div>
          ...
          {wishlist.map((item, i) => (
            <motion.div key={item.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
              ...
              <div className="p-4">
                ...
                <p className="text-xs capitalize mb-3" style={{ color: "var(--text-muted)" }}>{item.category}</p>
